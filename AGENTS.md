...
- Pocket Dungeon — `dungeon/` (`https://kory111111111111111111.github.io/R1-Apps/dungeon/`): permadeath one-room-at-a-time crawler. 7×7 tile rooms (32px ASCII-baked sprites), scroll cycles facing, side click steps/attacks, long press pack, shake waits. Seeded floors 1–8 with bump combat, chests, traps, and a floor-8 ogre. Local sim always; LLM flavors room/death lines only. Persists via `creationStorage.plain` (Base64) + `localStorage`.
