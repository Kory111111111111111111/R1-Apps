...
- `familiar/` — Tamagotchi-style companion creature (Bit). Persists name, trait (`clingy|chill|fussy|nocturnal`), hatch/stage (Egg -> Baby -> Teen -> Adult), care-shaped body (round/spiky/tall), neglect scar, secret handshake, bond level, backpack inventory, and hunger/happiness/energy via `creationStorage.plain` (Base64) + `localStorage` JSON. Interactive gameplay: **FEED** with 5-item food menu (Berry, Pizza, Candy, Pepper, Tea) and favorite/dislike tastes; **PLAY** with real-time "Catch the Berry" falling-item mini-game using scroll wheel; **WALK** scavenger adventure to forage for collectible artifacts in a **BAG** backpack viewer; **SLEEP** dream mode with live energy regen; **SIGN** 3-move handshake; **NAME** editor; and **CAM** peephole. Features petting/rubbing micro-animations (chomp, zoomies, tickle, panic, bounce), floating particles (hearts, stars, crumbs, Zzz), 8-bit retro Web Audio soundscapes, and 50+ contextual personality dialogue lines with rich LLM on-device voice support (`wantsR1Response`). Desktop: keyboard (`Arrows`, `Space/Enter`, `S` shake, `T` pet, `Escape`, `L` speak), mouse wheel, and touch hold.
