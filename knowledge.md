...
- `familiar/` — Tamagotchi-style creature (Bit). Persists name, trait (`clingy|chill|fussy|nocturnal`), hatch/stage, care-shaped body (round/spiky/tall), neglect scar, secret handshake, day count, and hunger/happiness/energy via `creationStorage.plain` (Base64) + `localStorage` JSON. First FEED/PLAY/SLEEP hatches from egg to baby; teen ~6h, adult ~24h. Scroll cycles FEED/PLAY/SLEEP/NAME/SIGN/CAM; side does the action; shake = wake/play or handshake token; long-press = optional on-device LLM line (`useLLM` + `wantsR1Response`, no journal) with canned fallback. Mic RMS soothes; camera is a peephole behind the pet; Web Audio chirps on care. Desktop: click cycles care, button/Enter acts, arrows cycle, L = speak / confirm name, Backspace in NAME mode.
