...
- `dungeon/` — Pocket Dungeon: permadeath one-room-at-a-time crawler (`index.html`, `css/styles.css`, `js/sprites.js`, `js/dungeon.js`, `js/app.js`). Familiar-style ASCII letter-matrix sprites baked to 32px tiles (not Familiar's pet art). 7×7 rooms, facing+bump combat, seeded 8 floors, pack items, shake-wait. LLM narrates room/death JSON `{"line":"..."}` only; simulation is local. Persist `dungeonState` via `creationStorage.plain` (Base64) + `localStorage`.
