...
- `synth/` — first-party Creation: mini polyphonic synth (`index.html`, `css/styles.css`, `js/app.js`). Web Audio oscillator + low-pass filter, 8-pad C-major keyboard, scroll = cutoff, side button = waveform cycle, long-press panic, `creationStorage.plain` (Base64) plus `localStorage` (raw JSON) backup for wave/cutoff/octave.
