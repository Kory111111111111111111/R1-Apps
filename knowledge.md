...
- `dungeon/` — Pocket Dungeon: permadeath one-room-at-a-time crawler (`index.html`, `css/styles.css`, `js/sprites.js`, `js/dungeon.js`, `js/app.js`). Familiar-style ASCII letter-matrix sprites baked to 32px tiles (not Familiar's pet art). 7×7 rooms; tap a tile to path there (one turn per step). Side click waits (skip your step, enemies still act); long press pack; tap self also waits. Seeded 8 floors, bump combat, chests, traps, floor-8 ogre. LLM narrates room/death JSON `{"line":"..."}` only; simulation is local. Persist `dungeonState` via `creationStorage.plain` (Base64) + `localStorage`.
...
- Pocket Dungeon: `https://kory111111111111111111.github.io/R1-Apps/dungeon/`
